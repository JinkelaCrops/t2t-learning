...
def decode(estimator, hparams, decode_hp):
    if FLAGS.decode_interactive:
        ...
        decoding.decode_from_dataset(
            ...
            dataset_split="medicine.sample.txt" if FLAGS.eval_use_test_set else None)
...
def main(_):
    ...
    single_string = "因 盆腔 粘连 严重 或 输卵管 伞 端 闭锁 改行 标准 腹腔镜 检查 NumericInteger 例 , 占 PercentDecimal ( NumericInteger / NumericInteger ) ; 无 直肠 损伤 、 术后 穿刺 部位 出血 、 盆腔 感染 等 并发症 发生 。"
    sorted_inputs = [single_string] * 100
    ...
    for result in result_iter:
        decoded_outputs, _, _ = log_decode_results(result["inputs"],
