...
DATA_DIR=$HOMEPATH/t2t_data/new_medicine_new                                    # vocab
TRAIN_DIR=$HOMEPATH/t2t_train/new_medicine_new/$PROBLEM/$MODEL-$HPARAMS         # train model

# Decode
DECODE_FILE=$TMP_DIR/seg.$FILE_NAME.en.decode
OUTPUT_FILE=$TMP_DIR/seg.$FILE_NAME.zh.decode.translation
...
python mybleu.py -rf $TMP_DIR/seg.$FILE_NAME.zh.decode \
                 -tf $OUTPUT_FILE \
                 -l zh --lower True
# ==============================================
# end decoder
...
python analyzerdecode.py -f $TMP_DIR/seg.$FILE_NAME.zh.decode.translation \
                         -d $TMP_DIR/$FILE_NAME.en.decode.dict \
                         -t $TMP_DIR/encode.seg.$FILE_NAME.zh.decode.translation \
                         --dict_type json
...
# encode and cal bleu
python analyzerdecode.py -f $TMP_DIR/seg.$FILE_NAME.zh.decode \
                         -d $TMP_DIR/$FILE_NAME.zh.decode.dict \
                         -t $TMP_DIR/encode.seg.$FILE_NAME.zh.decode \
                         --dict_type json

python mybleu.py -rf $TMP_DIR/encode.seg.$FILE_NAME.zh.decode \
                 -tf $TMP_DIR/encode.seg.$FILE_NAME.zh.decode.translation \
                 -l zh --lower True --truncate 1000
