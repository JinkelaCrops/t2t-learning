...
parser = argparse.ArgumentParser(description="mytranslate_eval.py")
...
src_file_name = file_name + "." + args.src_lan
tgt_file_name = file_name + "." + args.tgt_lan
src_file_encode_name = src_file_name + ".encode"
tgt_file_encode_name = tgt_file_name + ".encode"
src_file_encode_dict_name = src_file_name + ".encode.dict"
tgt_file_encode_dict_name = tgt_file_name + ".encode.dict"
src_file_encode_seg_name = src_file_encode_name + ".seg"
tgt_file_encode_seg_name = tgt_file_encode_name + ".seg"

segment_dict = {"en": SegmentNLTK, "zh": SegmentJieba}

tgt_file_encode_seg_translate_name = tgt_file_encode_seg_name + ".translate"
tgt_file_encode_seg_translate_decode_name = tgt_file_encode_seg_translate_name + ".decode"


def file_unpack():
    if not if_cover:
        if os.path.exists(file_dir + "/" + src_file_name) and os.path.exists(file_dir + "/" + tgt_file_name):
            print(f"unpack info: {src_file_name} and {tgt_file_name} exists, exit unpack")
            return 0
    ...
    print(f"unpack info: export to {src_file_name} and {tgt_file_name}")
    with open(file_dir + "/" + src_file_name, "w", encoding="utf8") as f:
        f.writelines(["%s\n" % src for src in src_lines])
    with open(file_dir + "/" + tgt_file_name, "w", encoding="utf8") as f:
        f.writelines(["%s\n" % tgt for tgt in tgt_lines])
    return 0


def file_analyze():
    if not if_cover:
        if all(os.path.exists(file_dir + "/" + ff) for ff in
               [src_file_encode_name, tgt_file_encode_name,
                src_file_encode_dict_name, tgt_file_encode_dict_name]):
            print(f"analyze info: "
                  f"{src_file_encode_name}, {src_file_encode_dict_name} and "
                  f"{tgt_file_encode_name}, {tgt_file_encode_dict_name} exist, exit analyze")
            return 0

    for f_name, f_encode_name, f_encode_dict_name in zip([src_file_name, tgt_file_name],
                                                         [src_file_encode_name, tgt_file_encode_name],
                                                         [src_file_encode_dict_name, tgt_file_encode_dict_name]):
        tokens = Token()
        lines = []
...
def file_segment():
    if not if_cover:
        if all(os.path.exists(file_dir + "/" + ff) for ff in
               [src_file_encode_seg_name, tgt_file_encode_seg_name]):
            print(f"segment info: "
                  f"{src_file_encode_seg_name} and {tgt_file_encode_seg_name} exist, exit segment")
            return 0

    for f_name, f_seg_name, lan in zip([src_file_encode_name, tgt_file_encode_name],
                                       [src_file_encode_seg_name, tgt_file_encode_seg_name],
                                       [args.src_lan, args.tgt_lan]):
        seg_lines = []
...
def translate():
    shutil.copy(file_dir + "/" + tgt_file_encode_seg_name, file_dir + "/" + tgt_file_encode_seg_translate_name)
